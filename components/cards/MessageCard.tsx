...
interface MessageCardProps {
  title: string;
  /** Full message — rendered inside the modal */
  message: string;
  /** Optional preview override; defaults to first ~220 chars of message */
  preview?: string;
}

export function MessageCard({ title, message, preview }: MessageCardProps) {
        ...
        <div className="w-full max-w-2xl">
          ...
          <div className="relative rounded-2xl bg-spotify-card border border-white/5 p-6 sm:p-8 overflow-hidden">
            ...
            <div className="flex items-center gap-3 mb-4 relative">
              ...
              <div>
                <p className="text-sm font-semibold text-white">De Léo</p>
                <p className="text-xs text-spotify-text-secondary">Para Ana</p>
              </div>
            </div>
            ...
            <div className="mt-6 relative">
              <Button onClick={() => setOpen(true)}>Mostrar mensagem</Button>
            </div>
          </div>
        </div>
      </CardContainer>
